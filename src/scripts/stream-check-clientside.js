...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   ✅ ONNX Runtime loaded: ${ort.version || 'unknown version'}`
      );
    } catch (error) {
      ...
      try {
        ...
        if (detections.length > 0) {
          detections.slice(0, 3).forEach((det, idx) => {
            console.log(
              `      📍 Detection ${idx + 1}: Class ${det.classId} (${(det.confidence * 100).toFixed(1)}%)`
            );
          });
        }
    ...
    const totalDetections = testResults.reduce(
      (sum, r) => sum + (r.detectionCount || 0),
      0
    );

    console.log(`   📊 Successful frames: ${successfulFrames}/3`);
    console.log(
      `   ⏱️  Average processing time: ${avgProcessingTime.toFixed(1)}ms`
    );
...
function preprocessImageToTensor(image) {
  ...
  return new (require('onnxruntime-node').Tensor)('float32', tensorData, [
    1,
    3,
    targetSize,
    targetSize,
...
function parseDetections(outputData, confidenceThreshold = 0.5) {
  ...
  for (let i = 0; i < outputData.length; i += 6) {
    const [x1, y1, x2, y2, confidence, classId] = Array.from(
      outputData.slice(i, i + 6)
    );
