...
const MODEL_PATH = '/object_detection_model/best.onnx';
const INPUT_SIZE = 640;

window.UploadTF = (function(){
  let videoEl, canvasEl, ctx;
  let session, stream, rafId = 0;
  let devices = [], currentDevice = null;
  let detecting = true;
  let onDetection;

  async function init({ videoEl: v, canvasEl: c, onDetection: cb } = {}){
    videoEl = v;
    canvasEl = c;
    onDetection = cb;
  ...
  async function detect(){
    ...
    try {
      ...
      const { tensor, ...params } = preprocess(videoEl);
      const result = await s.run({ images: tensor });
      const boxes = postprocess(result, params);
      if (onDetection) onDetection(boxes); else drawDetections(boxes);
    } catch (e){
  ...
  async function startCamera(deviceId){
    ...
    try {
      ...
      await videoEl.play();
      ctx = syncCanvasToVideo(videoEl, canvasEl);
  ...
  async function listDevices(){
    try {
      ...
      if (select){
        ...
        currentDevice = devices[0]?.deviceId || null;
      }
    } catch (e){
      reportError(ErrorCodes.CAMERA_PERMISSION, e);
    }
  ...
  async function switchCamera(){
    ...
    currentDevice = devices[(idx+1) % devices.length].deviceId;
  ...
  function bindControls(){
    ...
    if (toggleBtn) toggleBtn.addEventListener('click', () => {
      toggleDetection();
      toggleBtn.textContent = detecting ? 'Pause Detection' : 'Resume Detection';
    });
    if (select) select.addEventListener('change', e => startCamera(e.target.value));
  }

  return { init, startCamera, stopCamera, switchCamera, drawDetections };
})();
