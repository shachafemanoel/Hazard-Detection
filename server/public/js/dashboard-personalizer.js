...
class DashboardPersonalizer {
    ...
    createPersonalizationPanel() {
        ...
        if (headerButtons) {
            ...
            personalizeBtn.addEventListener('click', () => this.togglePersonalizationPanel());
    ...
    collectSettings() {
        this.settings.theme = document.getElementById('theme-select')?.value || 'dark';
        this.settings.chartType = document.getElementById('chart-type-select')?.value || 'bar';
        this.settings.autoRefresh = document.getElementById('auto-refresh-toggle')?.checked || false;
        this.settings.refreshInterval = (document.getElementById('refresh-interval')?.value || 30) * 1000;
        
        this.settings.notificationPreferences.newReports = document.getElementById('notify-new-reports')?.checked || false;
        this.settings.notificationPreferences.statusUpdates = document.getElementById('notify-status-updates')?.checked || false;
        this.settings.notificationPreferences.summary = document.getElementById('notify-summary')?.checked || false;
    }
    ...
    populateSettingsForm() {
        ...
        const notifyStatusUpdates = document.getElementById('notify-status-updates');
        ...
        if (refreshInterval) refreshInterval.value = this.settings.refreshInterval / 1000;
        if (notifyNewReports) notifyNewReports.checked = this.settings.notificationPreferences.newReports;
        if (notifyStatusUpdates) notifyStatusUpdates.checked = this.settings.notificationPreferences.statusUpdates;
        if (notifySummary) notifySummary.checked = this.settings.notificationPreferences.summary;
    ...
    updateFavoriteLocationsList() {
        ...
        this.settings.favoriteLocations.forEach((location, index) => {
            ...
            locationDiv.className = 'favorite-location-item d-flex justify-content-between align-items-center mt-1';
