...
/**
 * Resolve base URL with private-first connectivity and local-dev override
 */
async function resolveBaseUrl() {
  ...
  const local = process.env.HAZARD_API_URL_LOCAL || 'http://localhost:8080';
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    console.log(`🔒 Private network selected: ${local}`);
    return local;
  }

  if (pref === 'true') {
    console.log(`🔒 Private network selected: ${priv}`);
    return priv;
  }
  if (pref === 'false') {
    console.log(`🌐 Public network selected: ${pub}`);
    return pub;
  }
