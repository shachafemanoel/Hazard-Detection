...
function createRealtimeClient(config) {
  ...
  async function send(payload) {
    ...
    try {
      ...
      // Handle retry logic
      const transientError = ['ECONNRESET', 'ECONNABORTED', 'ETIMEDOUT'].some(
        code => error.code === code || (error.message || '').includes(code)
      );
      if (retryCount < maxRetries && transientError) {
        retryCount++;
        console.log(`🔄 Retry ${retryCount}/${maxRetries} in ${backoffMs}ms`);
        // Keep status connected so retry passes connection check
        setStatus('connected');
        ...
        // Final failure, surface error and mark disconnected
        setStatus('disconnected');
        emit('error', error);
      }
    }
